<script>
	...
	const changePage = async () => {
		try {
			await goto(`paket-wisata?page=${currentPage}&hari=${durasi}&destinasi=${destinasi}`, {
				replaceState: true,
				invalidateAll: true
			});
		} catch (error) {
				...
				<div class="col-12">
					<div class="bg-neutral-0 rounded-2 py-3 px-6 box-shadow">
						<div class="text-center">
							<h2 class="mt-10 mb-5">Blog tidak ditemukan</h2>
							<p class="mb-8">Mohon Maaf Blog Masih Dalam Proses Pengembangan</p>
						</div>
					</div>
				</div>
			{/if}
